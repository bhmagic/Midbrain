...
if (-not (Test-Path $python)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not create Generic Grip Skill environment." }
}
...
if ($LASTEXITCODE -ne 0) { throw "Generic Grip Skill installation failed." }
...
if (-not (Test-Path $profiles)) {
    ...
    if ($profileChanged) {
        ...
        Write-Host "Migrated unchanged generic grip defaults to the current close target, speed, and contact timeout."
    }
}
Write-Host "Generic Grip Skill environment ready: $venv"
Write-Host "Local generic grip motion profiles: $profiles"
