...
if (-not (Test-Path $python)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not create Let Go Skill environment." }
}
...
if ($LASTEXITCODE -ne 0) { throw "Let Go Skill installation failed." }
Write-Host "Let Go Skill environment ready: $venv"
