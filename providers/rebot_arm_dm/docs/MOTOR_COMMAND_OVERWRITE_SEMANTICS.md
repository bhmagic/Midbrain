...
Physical TRANSIT_SPEED uses a separate POS_VEL cap vector from the conservative MIT/calibration rate caps. The current caps are 5.0 rad/s for J1-J3 and 10.0 rad/s for J4-J6 and the gripper; every value remains bounded by that motor's configured VMAX. These values are selected inside the published motor characteristic-speed envelope, not qualified continuous-duty whole-arm speeds. Requested intent above 10 rad/s on any arm joint requires explicit authentication and intent at or above 20 rad/s is rejected before execution. These policy thresholds do not raise the effective motor cap. Changing the POS_VEL vector does not alter MIT target-rate validation or automatic-calibration excitation limits.
