...
if (-not (Test-Path $python)) {
    if ($PythonLauncher -eq "py") { & py -3.11 -m venv (Join-Path $workspace ".venv") }
    else { & $PythonLauncher -m venv (Join-Path $workspace ".venv") }
}
...
Write-Host "Local VIO provider setup complete."
