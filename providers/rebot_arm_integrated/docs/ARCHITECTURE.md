...
Fabric is a target source in 0.8.1, not a motion-authority mechanism. It distinguishes absolute `ik_location`, tool-to-acting-point `ik_offset`, and base-frame geometric `ik_gravity_offset`. Operator debug motion still requires Engage + LB. Agentic transit instead requires a signed decision-specific assertion and the controller's exact current preview.
