...
function Invoke-ComponentSetup {
    ...
    if (Test-Path -LiteralPath $setupPath) {
        ...
        Write-Host "[$Step/9] $Label package not present; skipped" -ForegroundColor Yellow
    }
}

Write-Host "[1/9] Setting up Manager and Fabric"
& (Join-Path $PSScriptRoot "setup.ps1")

$providerSetup = Join-Path $workspace "providers\orbbec_femto_bolt\scripts\setup.ps1"
if (Test-Path $providerSetup) {
    Write-Host "[2/9] Setting up Orbbec Femto Bolt Provider"
    & $providerSetup `
    ...
    Write-Host "[2/9] Orbbec Provider package not present; skipped" -ForegroundColor Yellow
}

Invoke-ComponentSetup 3 "Setting up Local VIO Provider" `
    "providers\local_vio\scripts\setup.ps1"
Invoke-ComponentSetup 4 "Setting up Spatial Registration Skill" `
    "skills\spatial_registration_rgbd\scripts\setup.ps1"
Invoke-ComponentSetup 5 "Setting up Tool Registration Skill" `
    "skills\register_tool_to_control_frame\scripts\setup.ps1"
Invoke-ComponentSetup 6 "Setting up Effector-Front Skill" `
